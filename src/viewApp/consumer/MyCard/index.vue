<template>
        <div class="myCard-container">
                <div class="myCard">
                        ...
                        <!-- <div class="btn_add" @click="wxTitle" v-show="noCardShow || pageShow">
                                <img :src="require('@src/assets/images/card_add.png')" alt="">
                        </div> -->
                        <mt-button class="importBtn" v-if="cards.length" @click="wxTitle" type="danger">选择发票抬头</mt-button>
                </div>
...
import ShareTip from "@src/componentsApp/ShareTip";
...
export default {
        ...
        mounted() {
                ...
                this._czcEvent("我的名片进入页面");
                getWxSign("wx_consumer")({ url: window.location.href.split("#")[0] }, () => {
                        this.wxTitle();
                });
                // this.setCard({
                //   "title": "个人",
                //   "taxNumber": "345678765354576",
                //   "telephone": "010-21231222",
                //   "companyAddress": "注册地址",
                //   "bankName": "中国银行",
                //   "bankAccount": "8888888888888"
                // })
        },
        methods: {
                _initSwiper() {
                ...
                //获取微信抬头
                wxTitle() {
                        this.chooseInvoiceTitle((title) => { this.setCard(title) });
                },

                //设置名片
                setCard(title) {
                        let card = {};
                        card["enterpriseName"] = title.title || "";
                        card["taxNo"] = title.taxNumber || "";
                        card["enterpriseAddress"] = title.companyAddress || "";
                        card["enterprisePhone"] = title.telephone || "";
                        card["bankAddress"] = title.bankName || "";
                        card["bankAccount"] = title.bankAccount || "";
                        //创建二维码
                        this.createQr(title).then(qrcode => {
                                card["qr"] = qrcode || "";
                                this.cards = [card];
                ...
                chooseInvoiceTitle(callback) {
                        ...
                        //微信版本大于6.5.9才能使用导入微信名片功能
                        let version = utils.getWxVersion();
                        if (!utils.isVersion(version, "6.5.9")) {
                                this.Toast("您的微信版本过低，无法使用此功能哦~");
                                return;
                        }

                        wx.invoke("chooseInvoiceTitle", { scene: "0" }, res => {
                                if (res.err_msg == "chooseInvoiceTitle:ok") {
                                        var invoiceInfo = JSON.parse(res.choose_invoice_title_info);
                                        /**
                                         * invoiceInfo.type
                                         * type 0:企业  1:个人
                                         */
                                        this._czcEvent("我的名片获取成功");
                                        callback(invoiceInfo);

                                } else if (res.err_msg == "chooseInvoiceTitle:fail") {

                                        this._czcEvent("我的名片获取失败", `原因：${JSON.stringify(res)}`);
                                        this.Toast("获取失败");
                                } else if (res.err_msg == "chooseInvoiceTitle:cancel") {

                                        this._czcEvent("我的名片用户取消", "");
                ...
                //创建二维码
                createQr({ title, taxNumber, companyAddress, telephone, bankName, bankAccount }) {
                        let str = `${title}</>${taxNumber}</>${companyAddress} ${telephone}</>${bankName} ${bankAccount}</>`;
                        let CRC = crc.crc16(str.substr(4));
                        let base64 = Base64.encode(str + CRC);
                        let resStr = `$01${base64}$`;
                        return QRCode.toDataURL(resStr, { width: 250, });
                }
        }
};
</script>
